...
# Отладка: выводим список интерфейсов
echo "Список интерфейсов после фильтрации:"
echo "$interfaces"
...
for iface in $interfaces; do
    ...
    # Отладка: показываем IP-адрес для каждого интерфейса
    echo "Интерфейс $iface, IP-адрес: $ip_address"
...
# Отладка: показываем выбранный интерфейс до удаления лишнего номера
echo "Выбранный интерфейс до очистки: $net_interface"

# Если в переменной net_interface есть лишний номер, убираем его
net_interface=$(echo "$net_interface" | sed 's/^[0-9]*: //')

# Отладка: показываем выбранный интерфейс после очистки
echo "Выбранный интерфейс после очистки: $net_interface"
